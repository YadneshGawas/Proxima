...
export function RegistrationModal({ hackathon, open, onOpenChange }: RegistrationModalProps) {
  const { user } = useAuth();
  const { toast } = useToast();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [teamOption, setTeamOption] = useState<'existing' | 'temporary'>('existing');
  const [teamName, setTeamName] = useState('');
  const [contactEmail, setContactEmail] = useState(user?.email || '');
  const [contactPhone, setContactPhone] = useState('');
  const [teamMembers, setTeamMembers] = useState<TeamMember[]>([
    { name: user?.name || '', email: user?.email || '', role: 'Team Lead' },
  ]);

  // Get user's current team (mock - first team where user is a member)
  const userTeam = mockTeams.find(t => t.members.some(m => m.id === user?.id));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const effectiveMembers = useExistingTeam 
      ? userTeam.members.map(m => ({ name: m.name, email: m.email, role: m.role }))
      : teamMembers;

    if (hackathon.participationType === 'team' && effectiveMembers.length < hackathon.teamSize.min) {
      toast({
    ...
    try {
      await registrationService.register({
        ...
        userId: user.id,
        teamName: hackathon.participationType === 'team' ? effectiveTeamName : undefined,
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {hackathon.participationType === 'team' && (
            <div className="space-y-4">
              ...
              <RadioGroup value={teamOption} onValueChange={(v) => setTeamOption(v as 'existing' | 'temporary')}>
                {userTeam && (
                  <div className="flex items-start space-x-3 rounded-lg border border-border p-4">
                    ...
                    <Label htmlFor="existing" className="flex-1 cursor-pointer">
                      ...
                      <p className="mt-1 text-sm text-muted-foreground">
                        Register with "{userTeam.name}" ({userTeam.members.length} members)
                      </p>
                      {teamOption === 'existing' && (
          ...
          {(hackathon.participationType !== 'team' || teamOption === 'temporary') && hackathon.participationType === 'team' && (
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <Input id="contactPhone" type="tel" value={contactPhone} onChange={(e) => setContactPhone(e.target.value)} placeholder="+1-555-0123" required />
            </div>
          </div>

          {/* Team Members for Temporary Team */}
          {teamOption === 'temporary' && hackathon.participationType === 'team' && (
            <div className="space-y-4">
              ...
              {teamMembers.map((member, index) => (
                <div key={index} className="space-y-2 rounded-lg border border-border p-3">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium">{index === 0 ? 'Team Lead' : `Member ${index + 1}`}</span>
                    {index > 0 && (
                  ...
                  <div className="grid gap-2">
                    <Input placeholder="Name" value={member.name} onChange={(e) => updateTeamMember(index, 'name', e.target.value)} required />
                    <Input type="email" placeholder="Email" value={member.email} onChange={(e) => updateTeamMember(index, 'email', e.target.value)} required />
                    <Input placeholder="Role (e.g., Frontend Developer)" value={member.role} onChange={(e) => updateTeamMember(index, 'role', e.target.value)} required />
                  </div>
                </div>
              ))}
            </div>
